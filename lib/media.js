...
(function (Media) {
     ...
     * @enum {string}
     */
    Media.AvailableOutputFormats = AvailableOutputFormatsLeaf;
    /**
    ...
    class Helper {
        ...
        static async saveFromUrl(options) {
            ...
            if (blob.byteLength > maxDownloadBytes)
                throw new Error('Remote file is too large');
    ...
    Helper.preview = async (options) => {
        ...
        response.set('Vary', 'Accept-Encoding'); // Standard, safe edge-caching value
        response.set('Access-Control-Allow-Methods', 'GET, OPTIONS, HEAD');
        // Always apply these headers to GET, HEAD, and OPTIONS so the response signatures match perfectly
        response.set('Access-Control-Allow-Headers', 'Content-Type, Location, Content-Length');
        response.set('Access-Control-Max-Age', '3600');
        // Explicitly strip legacy, anti-caching headers often set by frameworks
        ...
        switch (request.method) {
            case 'GET':
            case 'HEAD':
                break; // Both methods now share the same exact header fingerprint
            case 'OPTIONS':
    ...
    Image.bufferImage = async (options) => {
        ...
        const convertToFormatsEnum = (str) => {
            const colorValue = Media.AvailableOutputFormats[str];
            return colorValue;
        };
        ...
        outputFormat ?? (outputFormat = Media.AvailableOutputFormats.jpeg);
        const animated = outputFormat === Media.AvailableOutputFormats.gif;
        let optionsImage = {};
