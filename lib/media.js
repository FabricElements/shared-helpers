...
(function (Media) {
    ...
    Helper.preview = async (options) => {
        ...
        if (options.height)
            imageResizeOptions.maxHeight = options.height;
        if (options.width)
            imageResizeOptions.maxWidth = options.width;
        ...
        if (options.dpr)
            imageResizeOptions.dpr = options.dpr;
        if (options.format)
        ...
        if (options.quality)
            imageResizeOptions.quality = options.quality;
    ...
    Image.sizeObjectFromImageSize = (inputSize) => {
        const sizeBase = inputSize && Media.sizesOptionsArray.indexOf(inputSize) >= 0 ? inputSize : ImageSize.standard;
        return {
