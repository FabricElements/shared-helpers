 ...
 */
const allowedProtocols = Object.freeze(['http:', 'https:']);
/**
 ...
 */
const ipv4ToInt = (address) => address
    .split('.')
...
export const isBlockedAddress = (address) => {
    ...
    if (!isIPv6(address))
        return true; // Not a literal address — treat as unsafe.
    ...
    if (isZeroPrefix && groups[5] === 0 && groups[6] === 0 && groups[7] <= 1)
        return true;
    // `fc00::/7` unique-local.
 ...
 */
export const assertSafeOutboundUrl = async (url) => {
    let parsed;
    try {
        parsed = new URL(url);
    }
    ...
    if (isIPv4(hostname) || isIPv6(hostname)) {
        if (isBlockedAddress(hostname))
            throw new Error('Invalid URL: address is not allowed');
        return parsed;
    }
    let addresses;
    try {
...
//# sourceMappingURL=outbound-url.js.map