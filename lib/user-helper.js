...
const firestore = new firestore_helper_1.FirestoreHelper();
/**
 * UserHelper
 */
class UserHelper {
    ...
    constructor(config) {
        ...
        this.token = (context) => {
            ...
            if (!authHeader) {
                throw new firebase_functions_1.https.HttpsError('unauthenticated', 'Missing Authorization header');
            }
            const _token = authHeader.split(' ')[1];
            if (!_token || _token.length < 5) {
                throw new firebase_functions_1.https.HttpsError('unauthenticated', 'Missing or invalid token');
            }
            return _token;
        };
        /**
        ...
        this.roleUpdateCall = async (data) => {
            ...
            switch (data.type) {
                ...
                case 'remove':
                    updateGroup = {
                        [data.document]: firestore_1.FieldValue.delete(),
                    };
            ...
            if (updateClaims) {
                ...
                await (0, auth_1.getAuth)().revokeRefreshTokens(data.uid);
            }
            return;
        };
