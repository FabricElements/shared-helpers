...
const claimsEqual = (a, b) => {
    ...
    if (isArrayA) {
        ...
        for (let i = 0; i < listA.length; i++) {
            if (!claimsEqual(listA[i], listB[i]))
                return false;
        }
        return true;
    }
    ...
    if (prototypeA !== Object.prototype && prototypeA !== null)
        return false;
    if (prototypeB !== Object.prototype && prototypeB !== null)
        return false;
    const recordA = a;
    const recordB = b;
    const keysA = Object.keys(recordA);
    if (keysA.length !== Object.keys(recordB).length)
        return false;
    for (const key of keysA) {
        if (!Object.prototype.hasOwnProperty.call(recordB, key))
            return false;
        if (!claimsEqual(recordA[key], recordB[key]))
            return false;
    }
    return true;
};
...
(function (User) {
     ...
     */
    User.serverOnlyFields = Object.freeze([
        'account',
        'backup',
        'bcId',
        'bsId',
     ...
     * @see Helper.sanitizeProfile
     */
    User.creatableProfileFields = Object.freeze([
        'abbr',
        'ads',
        'avatar',
        'country',
    ...
    Helper.update = async (data, mainUrl) => {
        ...
        if (country)
            updateDataFirestore.country = country;
        let onboarding = {};
        // Update User Name
     ...
     */
    Helper.sanitizeProfile = (data) => {
        const sanitized = {};
        if (!data)
            return sanitized;
        for (const field of User.creatableProfileFields) {
    ...
    Helper.createUser = async (data) => {
        ...
        const formatNames = _a.formatUserNames(data);
        const user = {
            ..._a.sanitizeProfile(formatNames),
            role: 'user',
        };
        if (!user.name?.length) {
    ...
    Helper.roleUpdateCall = async (data) => {
         ...
         */
        const storedClaims = userRecord.customClaims ?? {};
        let userClaims = { ...storedClaims };
        ...
        let updateGroup; // Action for the group
        switch (data.type) {
            case 'add':
                updateGroup = {
                    [data.group]: _role,
        ...
        if (grouped) {
             ...
             */
            const nextGroups = { ...(userDoc.groups ?? {}) };
            if (data.type === 'remove') {
         ...
         * already-issued token can ever present a role or group set that disagrees with
         * the current claims.
         *
         * Revocation applies to grants as well as removals: a token minted before a grant
         * lacks the new claim and would silently deny the newly-granted authority until the
         * token expired naturally.  Callers that derive authorization from token claims
         * need the token replaced promptly in both directions — not only on removal.
         *
         * Cost: revoking refresh tokens signs the user out of all active sessions.  The
         * client must obtain a fresh token (re-authenticate) after any role or group
         * change before privileged operations will succeed.
         */
        /** Maximum byte length of a Firebase custom-claims payload (Firebase platform limit). */
        const FIREBASE_CLAIMS_SIZE_LIMIT = 1000;
        if (updateClaims) {
            const claimsSize = JSON.stringify(userClaims).length;
            if (claimsSize > FIREBASE_CLAIMS_SIZE_LIMIT) {
                throw new Error(`Custom claims payload is ${claimsSize} bytes, which exceeds the Firebase ` +
                    `limit of ${FIREBASE_CLAIMS_SIZE_LIMIT} bytes. ` +
                    'The group set is too large to publish as claims.');
            }
