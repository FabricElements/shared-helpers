  ...
  async flush(): Promise<void> {
    ...
    try {
      ...
      if (combined.length > maxRetention) {
        ...
        logger.error(
          `BigQuery Storage Write buffer overflow for "${this.dataset}.${this.table}": ` +
 ...
 */
function resolveBigQueryProjectId(projectId?: string): string | undefined {
 ...
 */
function createBigQueryClient(projectId: string | undefined): BigQuery {
 ...
 */
async function getTableMetadataWithRetry(
  bigquery: BigQuery,
  dataset: string,
  table: string,
  ...
  for (let attempt = 0; attempt <= metadataRetryDelaysMs.length; attempt++) {
    try {
      return (await tableHandle.getMetadata()) as unknown as [BigQueryTableMetadata];
    } catch (error: unknown) {
      lastError = error;
  ...
  const error = new Error(
    `BigQuery table metadata for "${dataset}.${table}" could not be read after ` +
...
/**
 * Metadata returned by the BigQuery table lookup that this writer needs.
 */
...
function isRetryableMetadataError(error: unknown): boolean {
  ...
  if (code === 403 || code === 401) return false;
  if (typeof code === 'number' && [404, 408, 429, 500, 502, 503, 504].includes(code)) return true;
  const message = error instanceof Error ? error.message : String(error);
  if (/permission denied|access denied|unauthori[sz]ed/i.test(message)) return false;
